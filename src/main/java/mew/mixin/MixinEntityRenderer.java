...
import mew.events.PickEvent;
import mew.events.RaytraceEvent;
import mew.events.Render3DEvent;
import mew.module.modules.*;
...
public abstract class MixinEntityRenderer {
    ...
    private float hurtCameraEffect(float float1) {
        if (Mew.moduleManager == null) {
            return float1;
        } else {
            NoHurtCam noHurtCam = (NoHurtCam) Mew.moduleManager.modules.get(NoHurtCam.class);
    ...
    private void a(
        ...
        if (Mew.moduleManager != null) {
            GhostHand event = (GhostHand) Mew.moduleManager.modules.get(GhostHand.class);
            if (event.isEnabled()) {
    ...
    private double v(Vec3 vec31, Vec3 vec32) {
        if (Mew.moduleManager == null) {
            ...
            return Mew.moduleManager.modules.get(ViewClip.class).isEnabled() ? (double) this.thirdPersonDistance : vec31.distanceTo(vec32);
        }
    }
    ...
    private Material x(Block block) {
        if (Mew.moduleManager == null) {
            ...
            return Mew.moduleManager.modules.get(ViewClip.class).isEnabled() ? Material.air : block.getMaterial();
        }
    }
    ...
    private boolean y(EntityLivingBase entityLivingBase, Potion potion) {
        if (potion == Potion.blindness && Mew.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) Mew.moduleManager.modules.get(AntiDebuff.class);
    ...
    private boolean c(EntityPlayerSP entityPlayerSP, Potion potion) {
        if (potion == Potion.confusion && Mew.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) Mew.moduleManager.modules.get(AntiDebuff.class);
