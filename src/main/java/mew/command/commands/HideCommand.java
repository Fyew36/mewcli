    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            if (module == null) {
                ...
                ChatUtil.sendFormatted(String.format("%s&o%s&r is already hidden in HUD&r", Mew.clientName, module.getName()));
            } else {
                module.setHidden(true);
                ChatUtil.sendFormatted(String.format("%s&o%s&r has been hidden in HUD&r", Mew.clientName, module.getName()));
            }
        } else {
            for (Module module : Mew.moduleManager.modules.values()) {
                module.setHidden(true);
            }
            ChatUtil.sendFormatted(String.format("%sAll modules have been hidden in HUD&r", Mew.clientName));
        }
    }
}
