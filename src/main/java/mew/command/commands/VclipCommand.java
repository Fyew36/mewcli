    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() >= 2) {
            ...
            try {
                ...
                ChatUtil.sendFormatted(String.format("%sClipped (%s blocks)", Mew.clientName, df.format(distance)));
            }
            return;
        }
        ChatUtil.sendFormatted(
                String.format("%sUsage: .%s <&odistance&r>&r", Mew.clientName, args.get(0).toLowerCase(Locale.ROOT))
        );
    }
}
