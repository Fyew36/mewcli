    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            ChatUtil.sendFormatted(
                    String.format("%sUsage: .%s &oload&r/&osave&r <&oname&r> | .%s &olist&r | .%s &ofolder&r", Mew.clientName, command, command, command)
            );
        } else {
            ...
            switch (sub) {
                ...
                case "reload":
                    if (args.size() < 3) {
                        ChatUtil.sendFormatted(
                                String.format("%sMissing config name (use '&odefault&r' or '&o!&r' to load default config)&r", Mew.clientName)
                        );
                        return;
                    }
                ...
                case "list":
                    try {
                        File[] configs = new File("./config/mewcli/").listFiles(FILE_FILTER);
                        if (configs == null) {
                            throw new Exception();
                        }
                        if (configs.length == 0) {
                            ChatUtil.sendFormatted(String.format("%sNo configs found (&o%s&r)&r", Mew.clientName, "./config/mewcli/"));
                        }
                        Arrays.sort(configs, LastModifiedFileComparator.LASTMODIFIED_REVERSE);
                        ChatUtil.sendFormatted(String.format("%sConfigs:&r", Mew.clientName));
                        ...
                        ChatUtil.sendFormatted(String.format("%sFailed to read (&o%s&r)&r", Mew.clientName, "./config/mewcli/"));
                    }
                    return;
                case "f":
                case "folder":
                ...
                case "directory":
                    try {
                        Desktop.getDesktop().open(new File("./config/mewcli/"));
                    } catch (Exception e) {
                        ChatUtil.sendFormatted(String.format("%sFailed to open (&o%s&r)&r", Mew.clientName, "./config/mewcli/"));
                    }
                    return;
                default:
                    ChatUtil.sendFormatted(String.format("%sInvalid argument (&o%s&r)&r", Mew.clientName, args.get(1)));
            }
        }
    }
}
