...
import mew.events.RightClickMouseEvent;
import mew.module.Module;
import mew.util.BlockUtil;
...
public class NoSlow extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (this.isEnabled() && this.isFloatMode()) {
            ...
            if (this.lastSlot != item && PlayerUtil.isUsingItem()) {
                this.lastSlot = item;
                Mew.floatManager.setFloatState(true, FloatModules.NO_SLOW);
            }
        } else {
            this.lastSlot = -1;
            Mew.floatManager.setFloatState(false, FloatModules.NO_SLOW);
        }
    }
    ...
    public void onRightClick(RightClickMouseEvent event) {
        if (this.isEnabled()) {
            ...
            if (this.isFloatMode() && !Mew.floatManager.isPredicted() && mc.thePlayer.onGround) {
