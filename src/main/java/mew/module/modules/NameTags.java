...
import mew.mixin.IAccessorRenderManager;
                ...
                if (entity instanceof EntityLivingBase
                    ...
                    if (!StringUtils.isBlank(EnumChatFormatting.getTextWithoutFormattingCodes(teamName))) {
                        ...
                        if (this.backgroundOpacity.getValue() > 0) {
                            ...
                            float bgX = (float) (-width) / 2.0F - 2;
                            float bgY = (float) (-mc.fontRendererObj.FONT_HEIGHT) - 2;
                            float bgW = (float) width + 4;
                            float bgH = mc.fontRendererObj.FONT_HEIGHT + 4;
                            RoundedUtils.drawRoundOutline(bgX, bgY, bgW, bgH, 3, 0.5f, textColor,
                        ...
                        if (entity instanceof EntityPlayer) {
                            ...
                            if (TeamUtil.isFriend((EntityPlayer) entity)) {
                                ...
                                float bgW = (float) width + 4;
                                float bgH = mc.fontRendererObj.FONT_HEIGHT + 4;
                                int friendColor = Mew.friendManager.getColor().getRGB();
                                Color fc = new Color(friendColor);
                                RoundedUtils.drawRoundOutline(bgX, bgY, bgW, bgH, 3, 1, new Color(0, 0, 0, 0), fc);
                            } else if (TeamUtil.isTarget((EntityPlayer) entity)) {
                                float bgX = (float) (-width) / 2.0F - 2;
                                float bgY = (float) (-mc.fontRendererObj.FONT_HEIGHT) - 2;
                                float bgW = (float) width + 4;
                                float bgH = mc.fontRendererObj.FONT_HEIGHT + 4;
                                int targetColor = Mew.targetManager.getColor().getRGB();
                                Color tc = new Color(targetColor);
                                RoundedUtils.drawRoundOutline(bgX, bgY, bgW, bgH, 3, 1, new Color(0, 0, 0, 0), tc);
                            }
                        }
                        GlStateManager.popMatrix();
                    }
