...
import mew.events.*;
import mew.management.RotationState;
import mew.mixin.IAccessorPlayerControllerMP;
...
public class KillAura extends Module {
    ...
    private boolean performAttack(float yaw, float pitch) {
        if (!Mew.playerStateManager.digging && !Mew.playerStateManager.placing) {
    ...
    private boolean canAttack() {
        if (this.inventoryCheck.getValue() && mc.currentScreen instanceof GuiContainer) {
            ...
            if (((IAccessorPlayerControllerMP) mc.playerController).getIsHittingBlock()) {
                ...
                AutoHeal autoHeal = (AutoHeal) Mew.moduleManager.modules.get(AutoHeal.class);
                if (autoHeal.isEnabled() && autoHeal.isSwitching()) {
                    return false;
                } else {
                    BedNuker bedNuker = (BedNuker) Mew.moduleManager.modules.get(BedNuker.class);
                    AutoBlockIn autoBlockIn = (AutoBlockIn) Mew.moduleManager.modules.get(AutoBlockIn.class);
                    if (bedNuker.isEnabled() && bedNuker.isReady()) {
                        return false;
                    } else if (Mew.moduleManager.modules.get(Scaffold.class).isEnabled()) {
                        return false;
    ...
    public void onUpdate(UpdateEvent event) {
        if (event.getType() == EventType.POST && this.blinkReset) {
            ...
            Mew.blinkManager.setBlinkState(false, BlinkModules.AUTO_BLOCK);
            Mew.blinkManager.setBlinkState(true, BlinkModules.AUTO_BLOCK);
        }
            ...
            if (attack) {
                    ...
                    switch (this.autoBlock.getValue()) {
                        case 0: // NONE
                            if (PlayerUtil.isUsingItem()) {
                                ...
                                if (!this.isPlayerBlocking() && !Mew.playerStateManager.digging && !Mew.playerStateManager.placing) {
                                    swap = true;
                                }
                            } else {
                                this.isBlocking = false;
                        ...
                        case 2: // SPOOF
                            if (this.hasValidTarget()) {
                                ...
                                if (Mew.playerStateManager.digging
                                        || Mew.playerStateManager.placing
                            ...
                            if (this.hasValidTarget()) {
                                if (!Mew.playerStateManager.digging && !Mew.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        ...
                                        case 1:
                                            if (this.isPlayerBlocking()) {
                                                if(Mew.moduleManager.modules.get(NoSlow.class).isEnabled()){
                        ...
                        case 5: // INTERACT
                            if (this.hasValidTarget()) {
                                int item = ((IAccessorPlayerControllerMP) mc.playerController).getCurrentPlayerItem();
                                if (mc.thePlayer.inventory.currentItem == item && !Mew.playerStateManager.digging && !Mew.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        case 0:
                                            if (!this.isPlayerBlocking()) {
                                                swap = true;
                        ...
                        case 8: // FAKE
                            ...
                            if (PlayerUtil.isUsingItem()
                                    ...
                                    && !Mew.playerStateManager.digging
                                    && !Mew.playerStateManager.placing) {
                                swap = true;
                            }
                    }
                ...
                if (this.isBoxInSwingRange(this.target.getBox())) {
                        ...
                        if (this.rotations.getValue() == 3) {
                            Mew.rotationManager.setRotation(rotations[0], rotations[1], 1, true);
                        }
            ...
            if (this.showTarget.getValue() != 0
                ...
                switch (this.showTarget.getValue()) {
                    case 1:
                        ...
                        break;
                    case 2:
                        color = ((HUD) Mew.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
                }
