...
import mew.events.WindowClickEvent;
import mew.mixin.IAccessorItemSword;
...
public class ChestStealer extends Module {
    ...
    private boolean isInvManagerRequire(ItemStack itemStack) {
        if (itemStack == null) return false;
        InvManager invManager = (InvManager) Mew.moduleManager.modules.get(InvManager.class);
                    ...
                    if (this.oDelay <= 0 && this.clickDelay <= 0) {
                        if (this.isEnabled() && this.isValidGameMode()) {
                            ...
                            if (mc.thePlayer.inventory.getFirstEmptyStack() == -1) {
                                if (!this.warnedFull) {
                                    ChatUtil.sendFormatted(String.format("%s%s: &cYour inventory is full!&r", Mew.clientName, this.getName()));
