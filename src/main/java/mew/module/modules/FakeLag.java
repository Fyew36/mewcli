...
import mew.events.UpdateEvent;
import mew.module.Module;
import mew.property.properties.BooleanProperty;
import mew.property.properties.ColorProperty;
...
public class FakeLag extends Module {
    ...
    @Override
    public String getCategory() { return "Latency"; }
    ...
    private boolean isDynamicBlinking() {
        return Mew.blinkManager.getBlinkingModule() == BlinkModules.FAKE_LAG;
    }
    ...
    private void startDynamicBlink() {
        ...
        Mew.blinkManager.setBlinkState(true, BlinkModules.FAKE_LAG);
    }

    private void stopDynamicBlink() {
        if (Mew.blinkManager.setBlinkState(false, BlinkModules.FAKE_LAG)) {
    ...
    private void dynamicMessage(String message) {
        if (dynamicDebug.getValue()) {
            mew.util.ChatUtil.sendFormatted(Mew.clientName + this.getName() + ": &7" + message);
        }
    }
