...
import mew.events.Render2DEvent;
import mew.events.TickEvent;
import mew.mixin.IAccessorGuiChat;
import mew.module.Module;
import mew.util.ColorUtil;
import mew.util.RenderUtil;
...
public class HUD extends Module {
    ...
    public void onTick(TickEvent event) {
        if (this.isEnabled() && event.getType() == EventType.POST) {
            this.activeModules = Mew.moduleManager.modules.values().stream().filter(module -> module.isEnabled() && !module.isHidden()).sorted(Comparator.comparingInt(this::getModuleWidth).reversed()).collect(Collectors.<Module>toList());
        }
    }
    ...
    public void onRender2D(Render2DEvent event) {
        if (this.chatOutline.getValue() && mc.currentScreen instanceof GuiChat) {
            ...
            if (Mew.commandManager != null && Mew.commandManager.isTypingCommand(text)) {
        ...
        if (this.isEnabled() && !mc.gameSettings.showDebugInfo) {
            ...
            for (Module module : this.activeModules) {
                ...
                float bgAlpha = this.background.getValue().floatValue() / 100.0F;
                float entryX = x / this.scale.getValue();
                float entryY = y / this.scale.getValue();
                float entryW = totalWidth + (this.showBar.getValue() ? 4 : 2);
                float entryH = height;

                if (bgAlpha > 0) {
                    float bgX = entryX - 1.0F;
                    float bgW = entryW + 3.0F;
                    if (this.posX.getValue() == 1) {
                        bgX = entryX - entryW - 2.0F;
                    }
                    RoundedUtils.drawRoundOutline(bgX, entryY - 1, bgW, entryH + 2, 2.5f, 0.5f,
                        new Color(0, 0, 0, (int)(bgAlpha * 255)),
                        new Color(color >> 16 & 0xFF, color >> 8 & 0xFF, color & 0xFF, 60));
                }

                if (this.showBar.getValue()) {
                    int barX = (int)(entryX - (this.posX.getValue() == 0 ? 1.0F : entryW - 1.0F));
                    int barW = 2;
                    RoundedUtils.drawRound(barX, entryY - 1, barW, entryH + 2, 1, true, new Color(color));

                    Color darker = new Color((color & 0xFEFEFE) >> 1 | color & 0xFF000000);
                    RoundedUtils.drawRound(barX, entryY, barW, entryH, 1, true, darker);
                }

                if (this.shadow.getValue()) {
                    mc.fontRendererObj.drawStringWithShadow(
                        moduleName,
                        entryX - (this.posX.getValue() == 1 ? entryW : 0.0F),
                        entryY,
                        color
                    );
                } else {
                    mc.fontRendererObj.drawString(
                        moduleName,
                        (int) (entryX - (this.posX.getValue() == 1 ? entryW : 0.0F)),
                        (int) entryY + (this.posY.getValue() == 1 ? 1 : 0),
                        color,
                        false
                    );
                }
                ...
                if (this.suffixes.getValue() && moduleSuffix.length > 0) {
                    ...
                    for (String string : moduleSuffix) {
                        if (this.shadow.getValue()) {
                            mc.fontRendererObj.drawStringWithShadow(
                                string,
                                entryX - (this.posX.getValue() == 1 ? entryW : 0.0F) + width,
                                entryY,
                            ...
                            mc.fontRendererObj.drawString(
                                string,
                                (int) (entryX - (this.posX.getValue() == 1 ? entryW : 0.0F) + width),
                                (int) entryY + (this.posY.getValue() == 1 ? 1 : 0),
                                ChatColors.GRAY.toAwtColor(),
                                false
                            );
            ...
            if (this.blinkTimer.getValue()) {
                BlinkModules blinkingModule = Mew.blinkManager.getBlinkingModule();
                if (blinkingModule != BlinkModules.NONE && blinkingModule != BlinkModules.AUTO_BLOCK) {
                    long movementPacketSize = Mew.blinkManager.countMovement();
