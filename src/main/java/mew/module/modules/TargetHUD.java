...
import mew.util.TeamUtil;
import mew.util.TimerUtil;
    ...
    private Color getTargetColor(EntityLivingBase entityLivingBase) {
        if (entityLivingBase instanceof EntityPlayer) {
            if (TeamUtil.isFriend((EntityPlayer) entityLivingBase)) {
                return Mew.friendManager.getColor();
            }
            if (TeamUtil.isTarget((EntityPlayer) entityLivingBase)) {
                return Mew.targetManager.getColor();
            }
        }
        switch (this.color.getValue()) {
            ...
            case 1:
                int rgb = ((HUD) Mew.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis()).getRGB();
    ...
    public void onRender(Render2DEvent event) {
        if (this.isEnabled() && mc.thePlayer != null) {
            ...
            if (this.target != null) {
                ...
                Color bgColor = new Color(0, 0, 0, (int)(bgAlpha * 255));
                Color outlineC = this.outline.getValue() ? targetColor : new Color(0, 0, 0, 0);

                RoundedUtils.drawRoundOutline(0, 0, barTotalWidth, 27, 4, 1, bgColor, outlineC);

                float barX = headIconOffset + 3;
                float barY = 21;
                float barW = barTotalWidth - barX - 3;
                float barH = 5;
                float fillW = barW * healthRatio;

                RoundedUtils.drawRound(barX, barY, barW, barH, 2, true, ColorUtil.darker(healthBarColor, 0.2f));
                RoundedUtils.drawRound(barX, barY, fillW, barH, 2, true, healthBarColor);

                GlStateManager.enableBlend();
                GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
                mc.fontRendererObj.drawString(targetNameText, (int)(headIconOffset + 4), 2, -1, this.shadow.getValue());
                mc.fontRendererObj.drawString(healthText, (int)(headIconOffset + 4), 11, -1, this.shadow.getValue());
                if (this.indicator.getValue()) {
                    mc.fontRendererObj.drawString(statusText, barTotalWidth - 4 - (float) statusTextWidth, 2, healthDeltaColor.getRGB(), this.shadow.getValue());
                    mc.fontRendererObj.drawString(healthDiffText, barTotalWidth - 4 - (float) healthDiffWidth, 11, ColorUtil.darker(healthDeltaColor, 0.8F).getRGB(), this.shadow.getValue());
                }
                if (this.head.getValue() && this.headTexture != null) {
                    GlStateManager.color(1.0F, 1.0F, 1.0F);
                    mc.getTextureManager().bindTexture(this.headTexture);
                    Gui.drawScaledCustomSizeModalRect(2, 1, 8.0F, 8.0F, 8, 8, 23, 23, 64.0F, 64.0F);
                    Gui.drawScaledCustomSizeModalRect(2, 1, 40.0F, 8.0F, 8, 8, 23, 23, 64.0F, 64.0F);
                    GlStateManager.color(1.0F, 1.0F, 1.0F);
                }
