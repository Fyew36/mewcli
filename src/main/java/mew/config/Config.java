...
public class Config {
    ...
    public Config(String name, boolean newConfig) {
        ...
        this.file = new File("./config/mewcli/", String.format("%s.json", this.name));
        try {
    ...
    public void load() {
        try {
            ...
            if (!file.exists()) {
                ChatUtil.sendFormatted(String.format("%sConfig file not found (&c&o%s&r). Creating default config...&r", Mew.clientName, file.getName()));
                save();
                return;
            }
            ...
            if (parsed == null || !parsed.isJsonObject()) {
                ChatUtil.sendFormatted(String.format("%sInvalid config format (&c&o%s&r)&r", Mew.clientName, file.getName()));
                return;
            }
            ...
            for (Module module : Mew.moduleManager.modules.values()) {
                ...
                if (moduleObj != null && moduleObj.isJsonObject()) {
                    ...
                    ArrayList<Property<?>> list = Mew.propertyManager.properties.get(module.getClass());
                    if (list != null) {
            ...
            ChatUtil.sendFormatted(String.format("%sConfig has been loaded (&a&o%s&r)&r", Mew.clientName, file.getName()));
        } catch (FileNotFoundException e) {
            ChatUtil.sendFormatted(String.format("%sConfig file not found (&c&o%s&r)&r", Mew.clientName, file.getName()));
        } catch (JsonSyntaxException e) {
            ChatUtil.sendFormatted(String.format("%sConfig has invalid JSON syntax (&c&o%s&r)&r", Mew.clientName, file.getName()));
            ...
            ChatUtil.sendFormatted(String.format("%sConfig couldn't be loaded (&c&o%s&r)&r", Mew.clientName, file.getName()));
        }
    }
    ...
    public void save() {
        try {
            ...
            printWriter.close();
            ChatUtil.sendFormatted(String.format("%sConfig has been saved (&a&o%s&r)&r", Mew.clientName, file.getName()));
        } catch (IOException e) {
            ((IAccessorMinecraft) mc).getLogger().error("Error saving config: " + e.getMessage());
            ChatUtil.sendFormatted(String.format("%sConfig couldn't be saved (&c&o%s&r)&r", Mew.clientName, file.getName()));
        }
    }
}
